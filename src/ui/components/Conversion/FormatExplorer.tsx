...
export default function FormatExplorer({
    ...
    const [originalIndex, _setOriginalIndex] = useState<SearchIndex>(() =>
        generateSearchIndex(conversionOptions)
    );

    const [searchResultsIndex, setSearchResultsIndex] =
        useState<SearchIndex>(originalIndex);

    useEffect(() => {
        setSearchResultsIndex(originalIndex);
    }, []);
    ...
    const handleDebounceSearch = useDebouncedCallback((term) => {
        (!term)
            ? setSearchResultsIndex(originalIndex)
            : setSearchResultsIndex(filterFormats(originalIndex, term));
    }, debounceWaitMs);
